-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/ekta/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/ekta/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/ekta/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/ekta/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/ekta/outputs' USING PigStorage('\t');
